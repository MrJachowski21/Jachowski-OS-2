...
namespace JachowskiOS.System
{
    public static class ConsoleCommands
    {
        ...
        private static void CopyFile(string[] words)
        {
            if (words.Length < 2)
            {
                Console.WriteLine("Invalid syntax. Usage: copy <sourceFileName>");
                return;
            }

            string sourceFileName = words[1];
            ...
            try
            {
                string destinationPath = Path.Combine(destinationFolder, Path.GetFileName(sourceFileName));
                File.Copy(sourceFileName, destinationPath);
                Console.WriteLine($"File '{sourceFileName}' copied successfully to '{destinationPath}'.");
            }
            catch (Exception ex)
            {
